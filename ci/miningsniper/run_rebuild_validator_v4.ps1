...
if($first-lt0-or$last-lt0-or$first-eq$last){throw ('Expected distinct oldState/newState Clone occurrences, first='+$first+' last='+$last)}
$middle=$text.IndexOf($old,$first+$old.Length,[StringComparison]::Ordinal)
if($middle-ne$last){throw ('Expected exactly two Clone occurrences, middle='+$middle+' last='+$last)}
$patched=$text.Substring(0,$last)+$new+$text.Substring($last+$old.Length)
