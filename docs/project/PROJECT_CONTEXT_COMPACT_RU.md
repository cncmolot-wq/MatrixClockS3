...
- The current `s3` voice track also has a lab mode for custom command work: after wake word, firmware can capture a short post-wake command window into `LittleFS` as `voice_lab_last.wav`, so a lightweight project-specific recognizer can be trained on real microphone data without dragging full `MultiNet` into the product path.
