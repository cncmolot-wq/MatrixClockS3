...
- The current `s3` voice track also has a lab mode for custom command work: after wake word, firmware captures a post-wake command window into numbered `LittleFS` WAV files (`/voice_lab_0001.wav` ...), reuses free slots after deletions, and no longer hard-freezes the display present path during active capture.
