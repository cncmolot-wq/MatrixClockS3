...
- если важная политика или механизм добавлены в `PLAN` / `RULES`, их короткое пояснение тоже должно появиться в `README`
...
- не каждая локальная правка обязана обновлять все слои документации: `README` обновляется только если меняется человечески важный смысл, `PLAN` — если меняется техническое решение или статус, а `docs/chatgpt_git` — только если меняется внешний ChatGPT/GitHub workflow
...
---
## <span style="color:#4EA1FF;">Роль README</span>

- `README_RU.md` — описательный слой проекта.
...
Этот README синхронизирован с [`REFACTOR_PLAN_RU.md`](./REFACTOR_PLAN_RU.md) по следующим точкам:
