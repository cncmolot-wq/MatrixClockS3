...
- The current `s3` voice track also has a lab mode for custom command work: after wake word, firmware captures a post-wake command window into numbered `LittleFS` WAV files (`/voice_lab_0001.wav` ...), reuses free slots after deletions, and keeps the display frozen during active capture so the matrix does not shimmer while samples are being recorded.
- Build modes are now intentionally separated: `env:s3` is the product voice path, while `env:s3_voice_debug` carries lab-only flags such as wake-only trigger and WAV capture. Product tuning must happen against `s3`, not against a laboratory env by accident.
...
- Voice/debug build-mode changes: run both `platformio run -e s3` and `platformio run -e s3_voice_debug`.
