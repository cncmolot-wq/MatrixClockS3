...
- The shared microphone policy is now explicit, and `voice` already has VAD + speech-segment telemetry + command-slot + queued HTTP-action groundwork, but real phrase recognition/intent parsing is still not implemented; future work must stay on the service layer instead of bypassing `audio_capture_service.*` or mixing speech parsing directly into transport code.
