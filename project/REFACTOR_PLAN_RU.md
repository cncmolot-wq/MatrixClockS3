...
- `Stage 2` — <span style="color:#67C587;">частично выполнен</span>
  ...
  - `Fire` playback вынесен из `spectrum.cpp` в `audio/fire_animation.*`, сохранив preload `.anim` в RAM / PSRAM
...
- <span style="color:#FFB347;">Частично закрыто:</span>
  - продуктовый Wi-Fi contour уже собран по backend/frontend основам, но ещё не прошёл полноценную полевую проверку как finished onboarding flow
  - OTA dev-flow стабилизирован, но финальный переносимый path ещё не доведён
  - DS3231/AT24C32 runtime реализован и подтверждён на железе через `/state`; EEPROM пока остаётся detect-only и не используется как storage
  - `main.cpp` уже разгружен по-крупному, а очистка кодировочных и комментарных шрамов начата с runtime-critical зон, но файл ещё не дочищен целиком
- <span style="color:#FF6B6B;">Ещё открыто:</span>
  - финальная фиксация product-grade Wi-Fi onboarding как действительно завершённого пользовательского сценария, включая полевую проверку recovery-кнопки и полного возврата в setup AP
  - определить финальные UI/diagnostic wording для RTC, если статус RTC понадобится показывать человеку в Web UI
  - закрытие dev-only OTA IP stabilization более переносимой схемой
  - дочистка битых комментариев/кодировки в runtime-файлах как остаточного риска сопровождения
...
- 2026-04-22 — Fire playback вынесен из `spectrum.cpp` в отдельный модуль `audio/fire_animation.*`: уже существующий preload `.anim` в RAM / PSRAM сохранён без изменения поведения, но файловая анимация больше не спрятана внутри spectrum runtime. Это продолжает Stage 2 через разделение ответственности.
...
- RTC hardware validation пройдена; следующий шаг — принять решение по `Stage 1` gate: закрывать фундамент или сначала добить оставшиеся дефекты Wi-Fi/OTA/комментариев
- проверить отдельную вкладку `Weather` и сохранение локации после загрузки `LittleFS`; если отображение корректно, следующим отдельным шагом решить, как погода должна появляться на матрице
- не добавлять новые пользовательские фичи до финального решения по закрытию `Stage 1`
- в финальном контуре поддержки отдельно закрыть dev-only OTA stabilization: оставить machine-specific `platformio.local.ini` как dev-решение или заменить его на более переносимый discoverable OTA path
