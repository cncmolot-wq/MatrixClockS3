...
- Не каждая локальная правка обязана обновлять все документные слои: `README` обновляется по смыслу для человека, `PLAN` — по техническому статусу и решениям, `docs/chatgpt_git` — только при изменении внешнего workflow.
